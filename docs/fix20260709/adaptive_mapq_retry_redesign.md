...
| **证据不完整** | `incomplete-of8-fallback` | top 8 中只有 1-7 条 rRNA 产生有效投票 | ✅ 增加丰度或有效投票 | **✅ 重试** |
| **数据冲突** | `4of8-split-fallback` | 恰好 8 条有效投票，且两类各 4 票 | ❌ 降低只会引入更多噪声 | ❌ 不重试 |
| **数据冲突** | `multi-of8-fallback` | 恰好 8 条有效投票，但为其他任何非 7/8 共识分布（如 6:2、5:3 或三类） | ❌ 降低只会引入更多噪声 | ❌ 不重试 |
...
### Level 4 fallback 的精确定义

Level 4 先按 top 8 rRNA 统计有效投票；单条 rRNA reads 少于 18 时为 `insufficient-data`，不计入有效投票。没有有效投票时保留 `all-insufficient-fallback`。有 1-7 条有效投票时输出 `incomplete-of8-fallback`，属于数据不足并继续重试。只有有效投票恰好为 8 条时，才按完整冲突处理：两类各 4 票为 `4of8-split-fallback`；其余未达到 7/8 共识的完整分布为 `multi-of8-fallback`。
...
4. **Level 4 证据分类与自适应 MAPQ 终止条件**：`incomplete-of8-fallback` 与其他数据不足 fallback 会降阈值重试；只有完整的 `4of8-split-fallback` 和 `multi-of8-fallback` 会终止。
