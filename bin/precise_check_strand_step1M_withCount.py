#!/usr/bin/env python3
"""
Strand bias analysis tool for NGS data - Precise version (Count-based)
...
# Import from fast_check_strand.py (shared module)
from fast_check_strand import determine_strandedness, compute_relative_difference
...
def analyze_counts(fwd: int, rev: int, label: str) -> CountBasedStats:
    ...
    # Use imported functions from fast_check_strand.py
...
def read_incremental_counts_file(filepath: str) -> List[Tuple[int, int, str]]:
    ...
    Data format (output from precise_count_sam_1M_increase.sh):
...
def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python precise_check_strand_step1M_withCount.py <file.counts.txt> [output.tsv]")
        print()
