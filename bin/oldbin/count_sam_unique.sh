...
# This script counts UNIQUELY MAPPED reads only (bowtie2: no XS tag).
...
# Only count uniquely mapped reads (NH:i:1)
awk '
BEGIN {
    total = 0;
    fwd = 0;
...
!/^@/ {
    ...
    # Check for unique alignment (bowtie2 specific)
    # If XS tag exists, it means there are secondary alignments, skip this read
    for (i = 12; i <= NF; i++) {
        if ($i ~ /^XS:i:/) {
            next
        }
